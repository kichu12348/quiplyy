...
async function uploadImageInChunks(fileUri, endPoint) {
  ...
  for (let i = 0; i < totalChunks; i++) {
    ...
    const { done, success, uri } = await uploadChunk(
      chunk,
      i,
      totalChunks,
      fileName,
    ...
    if (!success && !done) {
      // teeny weeny error causing pitch
...
export async function downloadFile(fileName, endPoint) {
  ...
  const downloadUrl = `${endPoint}/message/downloadFile?fileName=${encodeURIComponent(
    fileName
  )}`;
...
export async function downloadSongFromServer(endPoint, songId,imageId) {
  if (!endPoint || !songId || !imageId) return { success: false, songUri: null, imageUri: null };
  const downloadUrlSong = `${endPoint}/song/downloadFile?fileName=${encodeURIComponent(songId)}`;
  const downloadUrlImage = `${endPoint}/song/downloadFile?fileName=${encodeURIComponent(imageId)}`;
  const fileUriSong = `${FileSystem.documentDirectory}${songId}.mp3`;
  const fileUriImage = `${FileSystem.documentDirectory}${imageId}.png`;

  try {
    const downloadResultSong = await FileSystem.downloadAsync(
      downloadUrlSong,
      fileUriSong,
      {
        md5: false,
    ...
    const downloadResultImage = await FileSystem.downloadAsync(
      downloadUrlImage,
      fileUriImage,
      {
        md5: false,
    ...
    if (downloadResultSong.status === 200 && downloadResultImage.status === 200) {
      return {
        success: true,
        songUri: fileUriSong,
